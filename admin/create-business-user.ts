...
async function createBusinessUser() {
  try {
    ...
    const { data: newUser, error: createError } = await supabase.auth.admin.createUser({
      email: 'business@seychellescompass.com',
      password: 'Business123!',
      email_confirm: true,
      user_metadata: {
        full_name: 'Seychelles Business Owner',
        role: 'business'
      }
    });
    ...
    console.log('✅ Business user setup completed successfully!');
    console.log('Email: business@seychellescompass.com');
    console.log('Password: Business123!');
    console.log('Role: business');
    
  } catch (error: any) {
    console.error('Error creating business user:', error.message || error);
  }
}
